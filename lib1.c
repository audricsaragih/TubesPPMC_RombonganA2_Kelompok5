...
void interfaceLayout(component_t *c,char matrix[40][40][3], int baris, int kolom){ //Interface Input dari Layouting manual
    char coma;
    int i,j;
...
void printMatrix(char matr[40][40][3], int row, int col){ //Prosedur Menampilkan Array hasil Layouting Manual
       int i,j,k;
    for(k = 0; k<= col; k++) {
		printf("%d\t", k);
	}
